...
class ProbeMessageModel(nn.Module):
    ...
    #     """
    #     Network forward
    #     :param atom_types: atom types of (N,) # nodes
    #     :param atom_coord: atom coordinates of (N, 3) # atom_xyz
    #     :param grid: coordinates at grid points of (G, K, 3) # probe_xyz
    ...
    #     """
    # num_nodes = torch.scatter_add(0, batch, torch.ones_like(batch))
    # nodes = torch.zeros((torch.max(batch) + 1, torch.max(num_nodes))
    # for i in torch.max(batch) + 1:
    #   n = num_nodes[i]
    #   nodes[i, :n] = atom_types[batch == i]
    #
    # atom_xyz = torch.zeros((torch.max(batch) + 1, torch.max(num_nodes), 3))
    # for i in torch.max(batch) + 1:
    #   n = num_nodes[i]
    #   atom_xyz[i, :n] = atom_coord[batch == i]
    #
    # num_probes = torch.scatter_add(0, batch, torch.ones_like(batch))
    # probe_xyz = torch.zeros((torch.max(batch) + 1, torch.max(num_probes), 3))
    # for i in torch.max(batch) + 1:
    #   n = num_probes[i]
    #   probe_xyz[i, :n] = grid[batch == i]
    # 
    ...
    def forward(
        ...
        if compute_dori:
            ...
            ##
            norm_grad_2 = (
                torch.linalg.norm(dp_dxyz / torch.unsqueeze(probe_output, 2), dim=2)
                ** 2
            )
        ...
        if compute_hessian:
            ...
            hessian = torch.zeros(
                hessian_shape, device=probe_xyz.device, dtype=probe_xyz.dtype
            )
