...
def compute_zeta(density, atoms, dist_cutoff=np.inf):
    ...
    grid_pos = calculate_grid_pos(density, [0.0, 0.0, 0.0], atoms.get_cell()).reshape(
        -1, 3
    )
...
def supercell_atoms_positions(atoms):
    ...
    repeats = [
        ...
        0,
        1,
    ]  # only need one repeat around unit cell to test for closest atom
    repeat_offsets = np.array(
        [(x, y, z) for x in repeats for y in repeats for z in repeats]
    )
